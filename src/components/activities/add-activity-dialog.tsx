        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="h-12 rounded-2xl bg-[#0A1F1C] px-6 text-[#44C3B2] hover:bg-[#0A1F1C]/80 font-bold transition-all">
                    <Plus className="mr-2 h-5 w-5" /> Nova Atividade
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[900px] rounded-3xl border-white/50 bg-white/70 backdrop-blur-2xl shadow-2xl p-8 font-body border max-h-[90vh] overflow-y-auto">
                <DialogHeader className="space-y-3">
                    <DialogTitle className="text-3xl font-bold font-heading text-[#0A1F1C]">Nova Atividade</DialogTitle>
                    <DialogDescription className="text-[#0A1F1C]/60">
                        Registe uma nova experiência ou equipamento de lazer para os seus clientes.
                    </DialogDescription>
                </DialogHeader>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pt-4">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-[#0A1F1C]/50">Nome da Atividade</FormLabel>
                                    <FormControl>
                                        <Input placeholder="Ex: Aluguer de Seabob F5S" {...field} className="rounded-xl border-white/50 bg-white/50 focus-visible:ring-[#44C3B2]" />
                                    </FormControl>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-[10px] font-black uppercase tracking-widest text-[#0A1F1C]/50">Disponibilidade</FormLabel>
                                        <FormControl>
                                            <Input placeholder="Ex: Todo o ano" {...field} className="rounded-xl border-white/50 bg-white/50 focus-visible:ring-[#44C3B2]" />
                                        </FormControl>
                                        <FormMessage className="text-[10px]" />
                                    </FormItem>
                                )}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-[10px] font-black uppercase tracking-widest text-[#0A1F1C]/50">Unidades Disponíveis</FormLabel>
                                        <FormControl>
                                            <Input
                                                type="number"
                                                {...field}
                        ...
                        <div className="flex justify-end gap-3 pt-4">
                            ...
                            <Button type="submit" disabled={loading} className="h-12 rounded-2xl bg-[#0A1F1C] px-6 text-[#44C3B2] hover:bg-[#0A1F1C]/80 font-bold transition-all">
                                {loading ? "A guardar..." : "Registar Atividade"}
                            </Button>
                        </div>
                    </form>
                </Form>
